...
\title{Lecture 3: Bochner Integration and Stochastic Integration on Hilbert Spaces}
\date{30.10.2025}
\author{Tien Anh Vu}
...
We begin by fixing the Banach-space-valued setting in which the Bochner integral will be constructed. We work with a real Banach space \((E,\|\cdot\|)\), not necessarily separable, and a probability space \((\Omega,\mathcal A)\) equipped with a finite measure \(\mu\).
...
M=(M_t)_{t\in[0,T]} :
\begin{array}{l}
\text{\(M\) is an \(E\)-valued continuous \((\mathcal A_t)\)-martingale and} \\
\|M\|_{\mathcal M_T^p}:=\sup_{0\le t\le T}\left(\mathbb E\|M_t\|_E^p\right)^{1/p}<\infty
\end{array}
\right\}.
\]
In other words, this norm measures the \(L^p\)-size of the martingale uniformly over time.
...
The next objective is to define the stochastic integral with respect to the \(Q\)-Wiener process. Fix a \(U\)-valued \(Q\)-Wiener process \((W_t)\) with respect to a filtration \((\mathcal F_t)\). We want to define the stochastic integral of an operator-valued process \(\Phi(s)\) with respect to \(dW_s\), such that the resulting integral \(\int_0^T \Phi(s)\,dW_s\) takes values in a new Hilbert space \(H\).

Just as with the Bochner integral, we cannot immediately integrate general operator-valued processes. We proceed step by step.

\textbf{Step 1: Integration of elementary processes.}

We use a step function over a time partition to define an elementary process \(\Phi:[0,T]\times\Omega\to L(U,H)\), denoted as \((*)\). Let
\[
...
\]
Thus \(\Phi(t)\) is the elementary process itself, while the \(\Phi_m\) are the coefficients or building blocks of that process: each \(\Phi_m\) is an \(L(U,H)\)-valued random variable, and the whole collection of them defines the elementary process \(\Phi(t)\).
...
\[
I:\mathcal E\to \mathcal M_T^2,
\]
where \(\mathcal E\) denotes the space of elementary integrands \(\Phi\), and \(I(\Phi)\) is the martingale obtained from the stochastic integral of \(\Phi\).
...
\]
In the separable Hilbert-space setting of the lecture, this Hilbert-Schmidt space is again separable.
...
This equation states that the expected squared norm of the stochastic integral in the target space \(H\) is exactly equal to the expected time-integral of the squared Hilbert-Schmidt norm of the integrand composed with \(\sqrt{Q}\).

Equivalently,
\[
\left\|\int_0^t \Phi_s\,dW_s\right\|_{L^2(\Omega;H)}
=
\|\Phi\circ \sqrt Q\|_{L^2(\Omega\times[0,t];L_2(U,H))}.
\]
This shows the isometry directly: the stochastic integral preserves the \(L^2\)-norm.
...
The proof begins with the elementary processes already used in the definition of the stochastic integral. As in the script, we may assume without loss of generality that \(t=T\). We then start from the left-hand side of the isometry and expand it until the Hilbert-Schmidt norm appears.

By substituting the discrete definition of the elementary integral, we obtain
\[
\mathbb E\left(\left\|\int_0^t \Phi_s\,dW_s\right\|_H^2\right)
=
...
Now use the Hilbert-space identity
\[
\left\|\sum_{m=0}^{k-1} y_m\right\|_H^2
=
\left\langle \sum_{m=0}^{k-1} y_m,\sum_{n=0}^{k-1} y_n\right\rangle_H
=
\sum_{m,n}\langle y_m,y_n\rangle_H.
\]
Applying this with \(y_m=\Phi_m\Delta_m\) yields a double summation of inner products, and by linearity of expectation we may move this finite double sum outside the expectation:
\[
\mathbb E\left(\left\|\sum_{m=0}^{k-1}\Phi_m\Delta_m\right\|_H^2\right)
=
\sum_{m,n}\mathbb E\bigl(\langle \Phi_m\Delta_m,\Phi_n\Delta_n\rangle_H\bigr).
\]
To evaluate these cross-terms, we now expand each Wiener increment through the canonical representation of the \(Q\)-Wiener process. Let \((e_k)\) be a complete orthonormal system of \(U\) consisting of eigenvectors of \(Q\) with eigenvalues \(\lambda_k\), and define
\[
\beta_k(t):=\langle W(t),e_k\rangle_U.
\]
Then \((\beta_k)\) are the corresponding independent one-dimensional Brownian motions. Moreover, from
\[
W_t=\sum_{k=1}^{\infty}\sqrt{\lambda_k}\,\beta_k(t)e_k,
\]
we obtain
...
W_{t_{m+1}}-W_{t_m}
=
\sum_{k=1}^{\infty}\sqrt{\lambda_k}\,(\beta_k(t_{m+1})-\beta_k(t_m))e_k.
\]
Substituting these expressions for \(\Delta_m\) and \(\Delta_n\) into the inner products \(\langle \Phi_m\Delta_m,\Phi_n\Delta_n\rangle_H\), and then expanding with respect to the basis \((e_k)\), gives
\[
\sum_{m,n}\mathbb E\bigl(\langle \Phi_m\Delta_m,\Phi_n\Delta_n\rangle_H\bigr)
=
...
\]
What remains is to collapse this expression by the independence properties of the one-dimensional Brownian motions \((\beta_k)\). This is done in the next subsection.
...
We now analyze the cross-terms in the expansion obtained above. They involve expectations of products of Brownian increments of the form
\[
\mathbb E\bigl((\beta_k(t_{m+1})-\beta_k(t_m))(\beta_\ell(t_{n+1})-\beta_\ell(t_n))\mid \mathcal F_{t_{m\wedge n}}\bigr).
\]
To simplify the notation, write
\[
\Delta_m\beta_k:=\beta_k(t_{m+1})-\beta_k(t_m).
\]
Following the script, the vanishing terms are treated in two explicit cases.

\textbf{Case 1: \(m\neq n\).} Assume without loss of generality that \(m<n\). Then the later increment is independent of \(\mathcal F_{t_n}\), and therefore
\[
\mathbb E(\Delta_m\beta_k\,\Delta_n\beta_\ell\mid \mathcal F_{t_{m\wedge n}})
=
\Delta_m\beta_k\,\mathbb E(\Delta_n\beta_\ell\mid \mathcal F_{t_n})
=
0.
\]
Hence every term with \(m\neq n\) vanishes.

\textbf{Case 2: \(m=n\) but \(k\neq \ell\).} In this case the two increments belong to different one-dimensional Brownian motions. Since these Brownian motions are independent, we obtain
\[
\mathbb E(\Delta_m\beta_k\,\Delta_m\beta_\ell\mid \mathcal F_{t_{m\wedge n}})
=
\mathbb E(\Delta_m\beta_k\mid \mathcal F_{t_m})\,
\mathbb E(\Delta_m\beta_\ell\mid \mathcal F_{t_m})
=
0.
\]
Thus every term with \(m=n\) and \(k\neq \ell\) also vanishes.

Consequently, only the diagonal terms survive, namely those with \(m=n\) and \(k=\ell\). For these remaining terms, the expectation of the squared Brownian increment is
\[
\mathbb E\bigl((\beta_k(t_{m+1})-\beta_k(t_m))^2\bigr)
=
t_{m+1}-t_m.
\]
Hence, in the remaining diagonal case,
\[
\mathbb E\bigl(\langle \Phi_m(e_k),\Phi_n(e_\ell)\rangle_H\,\Delta_m\beta_k\,\Delta_n\beta_\ell\bigr)
=
\mathbb E\bigl(\|\Phi_m(e_k)\|_H^2\bigr)(t_{m+1}-t_m).
\]
After this reduction, the original quadruple sum collapses to the diagonal contribution that enters the next step.

\subsection*{6.2. Reconstructing the Hilbert-Schmidt Norm}
After the off-diagonal terms have vanished, the surviving diagonal terms take the form
\[
\sum_{k}\lambda_k\,\mathbb E\bigl(\|\Phi_m e_k\|_H^2\bigr)(t_{m+1}-t_m),
\]
for each fixed time interval \(m\). We now rewrite this expression so that the Hilbert-Schmidt norm appears explicitly.

Because
\[
\sqrt{\lambda_k}\,e_k=\sqrt{Q}\,e_k,
\]
we may absorb the factor \(\lambda_k\) into the operator evaluation and rewrite the diagonal contribution as
\[
\sum_k \lambda_k\,\mathbb E\bigl(\|\Phi_m e_k\|_H^2\bigr)(t_{m+1}-t_m)
=
\sum_k \mathbb E\bigl(\|\Phi_m(\sqrt{Q}\,e_k)\|_H^2\bigr)(t_{m+1}-t_m).
\]
Since \(\Phi_m(\sqrt{Q}\,e_k)=(\Phi_m\circ \sqrt{Q})e_k\), this is the same as
\[
\mathbb E\bigl(\|\Phi_m(\sqrt{Q}\,e_k)\|_H^2\bigr)
=
\mathbb E\bigl(\|\Phi_m\circ \sqrt{Q}\, e_k\|_H^2\bigr).
\]
Now the defining property of the Hilbert-Schmidt norm applies: summing the squared norms over a complete orthonormal system gives
\[
...
Because \(\Phi\) is elementary, it is constant on each interval \(]t_m,t_{m+1}]\). Therefore
\[
\int_0^t \|\Phi_s\circ \sqrt{Q}\|_{L_2(U,H)}^2\,ds
=
\sum_{m=0}^{k-1}\|\Phi_m\circ \sqrt{Q}\|_{L_2(U,H)}^2(t_{m+1}-t_m).
\]
...
This discrete formulation is exactly the Riemann-Lebesgue approximation of a time integral.
...
Conversely, if a positive operator on \(U\) has finite trace, then its square root is Hilbert-Schmidt.
...
\subsection*{A.12. \(L^p\)-Bounded Processes}
\begin{definition}[\(L^p\)-bounded process]
Let \((M_t)_{t\in[0,T]}\) be an \(E\)-valued process and let \(1\le p<\infty\). We say that \((M_t)\) is \(L^p\)-bounded if
\[
\sup_{0\le t\le T}\mathbb E\|M_t\|_E^p<\infty,
\]
or equivalently,
\[
\sup_{0\le t\le T}\left(\mathbb E\|M_t\|_E^p\right)^{1/p}<\infty.
\]
In words, the \(p\)-th moments of the process are uniformly bounded over the whole time interval \([0,T]\).
\end{definition}
