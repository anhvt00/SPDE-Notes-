...
Today, we will build some intuition by looking at a classic example that motivates the Dean-Kawasaki equation, originally introduced in the works of Dean in 1996 and Kawasaki in 1998. These ideas also connect to modern statistical models, where time reversal of stochastic differential equations plays an important role. We will start from a microscopic particle system and observe how it leads us to a macroscopic partial differential equation.
...
\section*{6. Stochastic Dynamics of the Empirical Measure}
So far, we have introduced the particle system, defined the empirical measure, and identified the heat equation as the macroscopic limit. We now examine the stochastic dynamics of the empirical measure and its fluctuations.

\subsection*{6.1. The Evolution Equation for the Empirical Measure}
...
\subsection*{6.2. Expanding into Integral Form and Density Assumptions}
...
\section*{7. Distributional Derivatives}
...
\section*{8. Fluctuations and the Strong Law of Large Numbers}
We now shift our focus to the fluctuations around the macroscopic limit as $N$ becomes large, but finite. Define the standardized fluctuation by
\[
...
\section*{9. The Space-Time Fluctuation Limit}
Finally, we formalize this fluctuation using space-time calculus:
\[
\text{where }\eta_t(x)\,dx=p(t,x)\,dx.
\]
...
\section*{10. The Central Limit Theorem for the Martingale Term}
We have derived the evolution equation for the empirical measure and identified the martingale term as the source of the fluctuations. We now study its fluctuation limit and connect the finite-particle system with the formal SPDE governing its fluctuations.

We now scale the martingale term $M_t^N(f)$ by $\sqrt{N}$ and examine its fluctuation limit:
\[
...
\[
\mathcal N\!\left(0,\int_0^t \eta_s\bigl((f')^2\bigr)\,ds\right).
\]
...
\section*{11. The Approximate Stochastic Differential Equation}
Synthesizing this, we write down the approximate SDE for our empirical measure:
\[
...
\section*{12. Space-Time White Noise and Variance Equivalence}
To move from a weak formulation tested against $f$ to a strong distributional formulation, we introduce space-time white noise $\xi(t,x)$ with covariance
\[
...
\section*{13. The Dean-Kawasaki Equation}
Recalling the earlier evolution equation
\[
d\eta_t^N(f)=dM_t^N(f)+\eta_t^N\!\left(\frac{1}{2}f''\right)dt,
\]
...
\subsection*{13.1. A Crucial Warning on Well-Posedness}
...
\section*{14. A Linear Comparison}
Finally, one may compare this with the linear equation
\[
dX_t=AX_t\,dt+CdW_t,
\]
...
\section*{15. Stochastic Analysis on Hilbert Space}
We now turn to a new foundational topic: stochastic analysis on Hilbert space. The goal is to move from finite-dimensional intuition to infinite-dimensional spaces and the probability measures defined on them.

\subsection*{15.1. The Setting: Hilbert Spaces and Operators}
We begin with the classical space $(L^2(\mu),\langle\cdot,\cdot\rangle)$ of square-integrable functions with respect to a measure $\mu$, equipped with its inner product. A Hilbert space is a complete normed vector space, where the norm is induced by the inner product and completeness means that every Cauchy sequence converges within the space.

We also consider linear operators acting between Hilbert spaces. If $L:U\to V$, where $U$ and $V$ are Hilbert spaces, then
\[
L \text{ is continuous} \iff \sup_{\|u\|_U\le 1}\|Lu\|_V<\infty.
\]
Thus, a linear operator is continuous if and only if it is bounded. This is a basic principle of functional analysis.

\subsection*{15.2. A Canonical Example: The $\ell^2$ Sequence Space}
To make things concrete, consider the space of square-summable sequences,
\[
\ell^2=\left\{(u_k)_{k\in\mathbb N}\in\mathbb R:\sum_{k=1}^\infty u_k^2<\infty\right\}.
\]
This is a fundamental example of an infinite-dimensional Hilbert space. Its inner product is given by
\[
\langle u,v\rangle=\sum_{k=1}^\infty u_kv_k,
\]
which is the infinite-dimensional analogue of the Euclidean dot product.

\subsection*{15.3. Separability}
A Hilbert space is called separable if there exists a countable dense subset $H_0\subset H$. This is an important topological requirement, because it implies that every element of the space can be approximated by a sequence from a countable set and, in particular, guarantees the existence of a countable orthonormal basis. This makes the construction of measures and integrals much more manageable.

\subsection*{15.4. Gaussian Measures on Hilbert Spaces}
We now move from deterministic functional analysis to probability theory.

\begin{definition}[Gaussian measure on a Hilbert space]
A probability measure $\mu$ on $(H,\mathcal B(H))$ is called a Gaussian measure if, for every $h\in H$, the linear functional
\[
...
Thus, a measure on a Hilbert space is Gaussian if every one-dimensional projection is a Gaussian random variable. By the Riesz representation theorem, every continuous linear functional on a Hilbert space can be represented as an inner product with some element $h\in H$, so this definition captures all continuous linear observations of the space.

\subsection*{15.5. The Characteristic Function}
The Gaussian property can be expressed through the characteristic function. There exist $m(h)\in\mathbb R$ and $\sigma^2(h)\in\mathbb R_+$ such that for all $t\in\mathbb R$,
\[
\int_H e^{it\ell_h(g)}\,\mu(dg)=e^{itm(h)-\frac12 t^2\sigma^2(h)}.
\]
This is exactly the characteristic function of a one-dimensional normal distribution with mean $m(h)$ and variance $\sigma^2(h)$.

The variance term $\sigma^2(h)$ has a quadratic dependence on $h$. This points toward the covariance operator, which encodes the variance structure of the Gaussian measure across the infinite-dimensional space.

\subsection*{15.6. Linearity of the Mean}
Finally, the mean functional is linear. Indeed,
\[
\int_H \ell_{h_1+h_2}(g)\,\mu(dg)=\int_H \ell_{h_1}(g)\,\mu(dg)+\int_H \ell_{h_2}(g)\,\mu(dg).
\]
Since the mean of the projection $\ell_h$ is given by its integral with respect to $\mu$, it follows that
\[
m(h_1+h_2)=m(h_1)+m(h_2).
\]

This prepares the way for studying Gaussian noise in infinite-dimensional settings, which will be essential when returning to objects such as space-time white noise in SPDEs.

\section*{16. Chapter 1: Stochastic Analysis on Hilbert Space}
We now shift our focus to a new, deeply foundational topic: stochastic analysis on Hilbert space. This part introduces the concepts of infinite-dimensional spaces and the measures defined on them.

\subsection*{16.1. The Setting: Hilbert Spaces and Operators}
The fundamental mathematical stage for this chapter is the Hilbert space
\[
(L^2(\mu),\langle\cdot,\cdot\rangle),
\]
which represents the classic $L^2$ space of square-integrable functions with respect to a measure $\mu$, equipped with an inner product. A Hilbert space is a complete normed vector space. The inner product induces the norm, and completeness means that every Cauchy sequence converges within the space.
...
\subsection*{16.2. A Canonical Example: The $\ell^2$ Sequence Space}
...
\subsection*{16.3. The Core Concept: Gaussian Measures on Hilbert Spaces}
We now move from deterministic functional analysis to probability theory.
...
\subsection*{16.4. The Characteristic Function of the Projection $\ell_h$}
...
\section*{17. Gaussian Measures on Hilbert Spaces}
We now continue from the previous discussion on defining Gaussian distributions via one-dimensional projections and formalize this with Theorem 1.6.

\subsection*{17.1. The Characteristic Function of the Projection $\langle g,h\rangle_H$}
...
The left-hand side is the characteristic function of the real-valued projection $\langle g,h\rangle_H$, and the right-hand side is the infinite-dimensional analogue of the one-dimensional Gaussian Fourier transform
\[
e^{itm-\frac12 t^2\sigma^2},
\]
...
Thus, this is the concrete Hilbert-space version of the abstract formula in 16.5, with $m(h)=\langle m,h\rangle_H$ and $\sigma^2(h)=\langle Qh,h\rangle_H$.

\subsection*{17.2. The Parameters: Mean Vector and Covariance Operator}
...
\subsection*{17.3. The Trace Class Requirement}
...
\subsection*{17.4. A Spectral Fact for Trace-Class Operators}
...
\subsection*{17.5. Three Fundamental Properties}
...
\]
This summarizes the trace computation above. It says that the squared norm of an element of the Hilbert space can be decomposed into the sum of the squared projections along an orthonormal basis.

\section*{18. Wiener Measure}
We now transition from the abstract definitions of Gaussian measures on Hilbert spaces to one of the most important concrete examples in probability theory: the Wiener measure.

\subsection*{18.1. The Path Mapping: Random Variables as Functions}
...
\subsection*{18.2. Measurability and the Induced Measure}
...
\subsection*{18.3. Proving that the Mean is Zero}
...
\section*{19. The Covariance Operator of the Wiener Measure}
We now turn to the second fundamental characteristic of the Wiener measure on the Hilbert space $L^2([0,T])$: the covariance operator $Q$. The goal is to identify this operator explicitly.

\subsection*{19.1. Formulating the Covariance}
...
\subsection*{19.2. Expanding into Double Integrals and Applying Fubini's Theorem}
...
\subsection*{19.3. Isolating the Covariance Operator $Q$}
...
\subsection*{19.4. Splitting the Integral Operator}
...
\section*{20. The Covariance Operator as the Inverse Laplacian}
The covariance operator of the Wiener measure can be identified with the inverse of the negative Laplacian.
