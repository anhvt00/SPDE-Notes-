...
\title{Lecture 12: Proof of Lemma 3.14 and Tightness}
\date{01.01.2026}
\author{Tien Anh Vu}
...
Let $(u_n)_n$ be a sequence such that
\[
...
\[
  u_n(0)\to u_0 \qquad \text{in } H.
\]
Then there exists a subsequence $(u_{n_k})_k$ and a limit $u$ such that
\[
  u_{n_k}\to u
...
\begin{proof}
Because we have equicontinuity in $V'$, there exists a subsequence $(u_{n_k})_k$ and some limit $u$ such that
\[
  u_{n_k}\to u
...
For the $V$-term, we do not have strong convergence in $V$. However, the sequence is uniformly bounded in $L^2([0,T];V)$, so
\[
...
\]
Thus the first term is bounded by a finite constant times $\varepsilon$.
